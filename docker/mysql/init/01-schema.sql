...
CREATE TABLE `stu_student` (
    ...
    `academic_status` VARCHAR(20) NOT NULL DEFAULT 'ENROLLED' COMMENT '学籍状态',
...
CREATE TABLE `crs_course_selection` (
    ...
    `status` VARCHAR(20) NOT NULL DEFAULT 'SELECTED' COMMENT '状态',
...
CREATE TABLE `grd_grade` (
    ...
    `letter_grade` VARCHAR(5) DEFAULT NULL COMMENT '等级',
    `grade_points` DECIMAL(3,2) DEFAULT NULL COMMENT '绩点',
    `grade_type` VARCHAR(20) NOT NULL DEFAULT 'NORMAL' COMMENT '成绩类型',
...
CREATE TABLE `lev_leave_request` (
    ...
    `duration` DECIMAL(5,1) NOT NULL COMMENT '请假时长',
...
CREATE TABLE `sys_audit_log` (
    ...
    `execution_time` BIGINT DEFAULT NULL COMMENT '执行时长',
